...
export default function UpdateChapterForm({
  ...
  const [currentSelectedWeekday, setCurrentSelectedWeekday] =
    useState<string>('')
  const [fieldNameWeekdayPairs, setFieldNameWeekdayPairs] = useState<
    FieldNameWeekdayPair[]
  >([])
  const [selectedWeekdays, setSelectedWeekdays] = useState([])

  const handleOptionChange = (fieldName: number, value: string) => {
    if (
      fieldNameWeekdayPairs.length === 0 ||
      !fieldNameWeekdayPairs.map((item) => item.fieldName).includes(fieldName)
    ) {
      setFieldNameWeekdayPairs([
        ...fieldNameWeekdayPairs,
