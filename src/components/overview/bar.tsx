...
export default function BarChart({ data }: BarChartsProps) {
  ...
  useEffect(() => {
    ...
    const teacherSkillsArr = Object.entries(data['teacher']).map(
      (item: [string, SkillStatistic[]]) => item[1]
    )
  ...
  return (
    <HighchartsReact highcharts={Highcharts} options={options} ref={charRef} />
  )
}
