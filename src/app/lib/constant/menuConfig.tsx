...
function getActiveKey(
  ...
  for (let menuItem of menuItems) {
    ...
    if (menuItem.path === path) {
      ...
      let [resMenuItem, resRecord] = getActiveKey(
        menuItem.subMenu,
        path,
        record
      )
...
export function getMenusByRole(role: string): MenuItem[] {
  let generatedMenus = addPathPrefixAndKey(
    rawMenus.get(role as Role)!,
    '/dashboard/' + role
  )
...
export function getDefaultKeys(
  ...
  const [activeMenuItem, activeRoute] = getActiveKey(
    generatedMenus,
    omitDetailPath(path),
    []
  )
