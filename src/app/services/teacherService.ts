...
class TeacherService extends BaseApiService {
  ...
  addTeacher(
    params: AddTeacherRequest
  ): Promise<IResponse<AddTeacherResponse>> {
    return this.post<IResponse<AddTeacherResponse>>('teachers', params).then(
      this.showMessage(true)
    )
  }
  deleteTeacher(id: number): Promise<IResponse<boolean>> {
    return this.delete<IResponse<boolean>>('teachers/' + id).then(
      this.showMessage(true)
    )
  }
  updateTeacher(
    params: UpdateTeacherRequest
  ): Promise<IResponse<UpdateTeacherResponse>> {
    return this.put<IResponse<UpdateTeacherResponse>>('teachers', params).then(
      this.showMessage(true)
    )
  }
}
