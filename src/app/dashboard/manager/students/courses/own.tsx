...
const OwnCoursesPage = () => {
  ...
  const columns: ColumnType<StudentOwnCourse>[] = [
    ...
    },
    {
      ...
      sorter: (a: StudentOwnCourse, b: StudentOwnCourse) =>
        a.course.name.charCodeAt(0) - b.course.name.charCodeAt(0),
  ...
  const {
    ...
    setLoading,
    setPaginator,
  } = useListEffect<CourseRequest, StudentOwnCoursesResponse, StudentOwnCourse>(
    courseService.getStudentOwnCourses.bind(courseService),
    'courses',
    true,
    { userId: storage?.userInfo?.userId }
  )
