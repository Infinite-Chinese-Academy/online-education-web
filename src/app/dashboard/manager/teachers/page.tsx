...
const Teachers = () => {
  ...
  const handleOk = () => {
    form.validateFields().then((values) => {
      if (isEditingTeacher) {
        ...
        teacherService.updateTeacher(updateTeacherRequest).then(() => {
          setModifyDataSuccessTimes(modifyDataSuccessTimes + 1)
        })
      } else {
        let addTeacherRequest: AddTeacherRequest = {
          ...values,
        }
        teacherService.addTeacher(addTeacherRequest).then(() => {
          setModifyDataSuccessTimes(modifyDataSuccessTimes + 1)
        })
      }
      form.resetFields()
  ...
  return (
    <AppLayout>
      ...
      >
        <Form form={form} labelCol={{ span: 6 }} wrapperCol={{ offset: 1 }}>
          <Form.Item
            label="Name: "
            name="name"
